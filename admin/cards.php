...
<head>
	...
	<title>All Virtual Cards - PartnersTrust CU</title>
							...
							<div class="table-responsive">
								<table id="example2" class="table table-bordered table-hover">
									...
									<tbody>
										<?php
										$count = 1;
										foreach ($Authroller->allCards() as $key => $value) {
											$user_info = $Authroller->singleUser($value['user_id']);
											?>
										<tr id="cardRow<?php echo $value['id']; ?>">
											...
											<td><?php echo date('h:i d/m/Y', strtotime($value['card_no'])); ?></td>
											<td class="text-capitalize"><?php echo $value['status']; ?></td>
											<td>
												<?php
												if ( $value['status'] == 'pending' ) {
													?>
												<a onclick="updateCard(<?php echo $value['id']; ?>, 'approved')" id="cardBtn<?php echo $value['id']; ?>" class="btn btn-primary btn-sm">Approve</a>
													<?php
												}elseif ( $value['status'] == 'hold' ) {
													?>
												<a onclick="updateCard(<?php echo $value['id']; ?>, 'approved')" id="cardBtn<?php echo $value['id']; ?>" class="btn btn-info btn-sm">Unfreeze</a>
													<?php
												}else {
													?>
												<a onclick="updateCard(<?php echo $value['id']; ?>, 'hold')" id="cardBtn<?php echo $value['id']; ?>" class="btn btn-danger btn-sm">Freeze card</a>
													<?php
												}
												?>
                        <!-- <a class="btn btn-danger btn-sm"
                          onclick="deleteCard(<?php echo $value['id']; ?>)"
